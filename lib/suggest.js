...
function suggestTokens(dictionary, text) {
  var _parser;
  var target = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'query';
  ...
  var scannables = parser.table[parser.current].scannable;
  var ret = parser ? unique(scannables.map(function (state) {
    return state.rule.symbols[state.dot];
  }).map(function (item) {
    if (item !== null && item !== void 0 && item.example) {
      if (!item.example.startsWith(prolog)) {
        return null;
      }
      return item.example.substr(prolog.length);
    }
    if (item !== null && item !== void 0 && item.literal) {
  ...
  if ((_parser = parser) !== null && _parser !== void 0 && _parser.results && parser.results.length > 0) {
    ret.push(eosToken);
  }
  return ret;
}
