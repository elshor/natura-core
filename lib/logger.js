"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;
function _typeof(obj) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, _typeof(obj); }
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, _toPropertyKey(descriptor.key), descriptor); } }
function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function _toPropertyKey(arg) { var key = _toPrimitive(arg, "string"); return _typeof(key) === "symbol" ? key : String(key); }
function _toPrimitive(input, hint) { if (_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
/**
 * Log to logger based on verbosity level. Can be one of error, warn, log, debug
 */
var Logger = /*#__PURE__*/function () {
  function Logger(logger) {
    var verbosity = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'error';
    _classCallCheck(this, Logger);
    this.verbosity = verbosity;
    this.logger = logger;
  }
  _createClass(Logger, [{
    key: "error",
    value: function error() {
      if (['error', 'warn', 'info', 'log', 'debug'].includes(this.verbosity)) {
        var _this$logger;
        (_this$logger = this.logger).error.apply(_this$logger, arguments);
      }
    }
  }, {
    key: "warn",
    value: function warn() {
      if (['warn', 'info', 'log', 'debug'].includes(this.verbosity)) {
        var _this$logger2;
        (_this$logger2 = this.logger).warn.apply(_this$logger2, arguments);
      }
    }
  }, {
    key: "log",
    value: function log() {
      if (['log', 'info', 'debug'].includes(this.verbosity)) {
        var _this$logger3;
        (_this$logger3 = this.logger).log.apply(_this$logger3, arguments);
      }
    }
  }, {
    key: "info",
    value: function info() {
      if (['log', 'debug', 'info'].includes(this.verbosity)) {
        var _this$logger4;
        (_this$logger4 = this.logger).info.apply(_this$logger4, arguments);
      }
    }
  }, {
    key: "debug",
    value: function debug() {
      if (['debug'].includes(this.verbosity)) {
        var _this$logger5;
        (_this$logger5 = this.logger).debug.apply(_this$logger5, arguments);
      }
    }
  }]);
  return Logger;
}();
exports["default"] = Logger;