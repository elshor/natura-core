...
var _packageDef = _interopRequireDefault(require("./package-def.js"));
...
var entities = [{
  ...
  properties: {
    name: {
      ...
      "default": function _default(spec) {
        if (spec && _typeof(spec) === 'object') {
          return spec.pattern;
        } else {
          return undefined;
        }
    ...
    pattern: {
      ...
      description: 'the pattern used to show the property'
    },
    emitOrder: {
      type: 'name*'
    },
    ...
    traitType: {
      type: 'name',
      "default": function _default(spec) {
        if (spec && _typeof(spec) === 'object' && (spec.name || spec.type)) {
    ...
    mustShow: {
      ...
      description: 'Determine if the "show" properties should be displayed' + ' after initializing the entity with the inline pattern.'
    },
    context: {
      type: 'context entry*'
    },
    ...
    title: {
      ...
      description: 'This will be used instead of the pattern when displaying suggestions'
    },
    actions: {
      type: 'entity action*',
      expanded: true
  ...
  properties: {
    ...
    trait: {
      ...
      type: function type(_ref) {
        ...
        var valueType = $location.parent.child('subject').valueType;
        if (!valueType) {
          return 'none';
        }
        return {
          $type: 'role type',
          type: "trait<".concat(valueType.searchString, ">"),
          role: 'type'
        };
      }
    }
  ...
  properties: {
    pattern: {
      type: 'pattern',
      placeholder: 'Type in the event pattern',
      description: 'The event pattern is how the pattern is refered to. E.g. user clicks on <<element>>'
    },
    description: {
      type: 'richtext'
    },
  ...
  properties: {
    ...
    useScope: {
      type: 'boolean',
      description: 'Use the scope of the emited object. If set to true then scope search iterates through the type scope entries or scope function'
    },
    description: {
      type: 'richtext'
    }
  ...
  properties: {
    property: {
      type: 'name',
      description: 'the name of the hash property'
    },
    proxyFor: {
      type: 'name',
      description: 'When the hash values are proxies for a property of theirs then need to also emit the proxy with the property name',
      title: 'proxy for'
    },
    access: {
      type: 'text'
  ...
  properties: {
    access: {
      type: 'text',
      placeholder: 'access to entity from scope'
    },
    path: {
      type: 'name',
      placeholder: 'path'
  ...
  properties: {
    ...
    hashSpec: {
      type: 'property spec',
      placeholder: 'Define type for dictionary objects'
    },
    "default": {
    ...
    init: {
      description: 'value to initialize the property with. The init value is only set at initialization of the object'
    },
    description: {
      type: 'richtext',
      placeholder: 'Description fo the property'
    },
    actions: {
      type: 'entity action*',
      expanded: true
    ...
    namePlaceholder: {
      ...
      placeholder: 'name to use for hash property'
    },
    childSpec: {
      type: 'property spec',
  ...
  properties: {
    base: {
      type: 'string',
      description: 'The base name to use. If this name does not exist in the context then just use it. If it exists then add an integer to it that does nott already appear'
    },
    path: {
      type: 'string',
      description: 'The path in which to check the uniqueness of the name'
    },
    type: {
      type: 'type',
      description: 'The context search type to use to evaluate which names are already in use'
    }
  }
}, {
  name: 'script',
  ...
  register: function register(dictionary, type, spec) {
  ...
  properties: {
    ...
    pattern: {
      type: 'pattern',
      placeholder: 'pattern to use for this action'
    },
    properties: {
      hashSpec: {
        type: 'property spec'
  ...
  properties: {
    ...
    icon: {
      ...
      description: 'name of material icon to use'
    },
    hint: {
      type: 'text'
    },
  ...
  name: 'call service',
  isa: ['editor action'],
  exec: function exec(context, _ref4) {
  ...
  show: ['topic', 'message'],
  isa: ['editor action'],
  exec: function exec(context, _ref5) {
    var postMessage = _ref5.postMessage,
      component = _ref5.component;
    function getLocations(message) {
      var paths = message.split(',').map(function (item) {
        return item.trim();
      }).map(function (item) {
        return item[0] !== '/' ? '/' + item : item;
      });
      var values = paths.map(function (path) {
        return calcPath(context, path);
      });
      return values.length === 1 ? values[0] : values;
    }
    var message = getLocations(this.message || '');
  ...
  properties: {
    ...
    message: {
      type: 'text',
      description: 'message to send. If message is not specified then send the triggering entity'
    }
  }
}, {
  name: 'hat action',
  ...
  properties: {
    ...
    hint: {
      type: 'text',
      description: 'The tooltip for the hat icon'
    }
  }
}, {
  ...
  properties: {
    isa: {
      ...
      "default": function _default(context) {
        ...
        var name = location.parent.parent.value.name;
  ...
  inlineClass: 'folder',
  //class used to display inline value
  properties: {
    name: {
      type: 'text',
  ...
  properties: {
    ...
    call: {
      ...
      type: 'string',
      noSuggestions: true,
      placeholder: 'Python function to call'
    }
  }
}, {
  name: 'js expression',
  ...
  properties: {
    subject: {
      type: 'text',
      readonly: true,
      inlineClass: 'text-bold',
      placeholder: 'trait subject'
    },
    title: {
      type: 'text'
    },
  ...
  properties: {
    ...
    title: {
      type: 'text',
      placeholder: 'title of parameter',
    ...
    placeholder: {
      type: 'strtexting',
      placeholder: 'placeholder to use for parameter value'
    }
  }
}, {
...
var _default2 = {
  ...
  entities: {
    $type: 'entity definition group',
    members: [].concat(entities, _toConsumableArray(_actions["default"]), _toConsumableArray(_validate.validators), _toConsumableArray(_packageDef["default"])),
    model: {
      isa: []
    }
  }
...
function calcValueType(_ref6) {
  var $location = _ref6.$location;
  //valueType of expression
  var expressionLocation = $location.sibling('expression');
  var valueTypeLocation = expressionLocation.child('valueType');
  var valueType = valueTypeLocation.value;
...
function defaultDefinitionModel(context) {
  ...
  var location = context.$location;
  if (_typeof(location.parent.value) !== 'object') {
    return {
      $type: 'definition model'
    };
  }
  var name = location.parent.value.name;
  if (typeof name !== 'string') {
    return {
      $type: 'definition model'
    };
  ...
  return {
    $type: 'definition model',
    isa: [location.lang.singular(name)]
  };
}
function calcPath(context, path) {
  ...
  var val = _jsonPtr.JsonPointer.get(context, path);
  var ret = val && _typeof(val) === 'object' && val.$isProxy ? val.$value : val;
  //check again if ret is a proxy then evaluate it. We do not do this recursivly so we don't get into infinite loop
  return ret && _typeof(ret) === 'object' && ret.$isProxy ? ret.$value : ret;
}