...
exports.Rule = Rule;
var _expandRule = require("./expand-rule.js");
...
function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) arr2[i] = arr[i]; return arr2; } /**
                                                                                                                                                                                     ...
                                                                                                                                                                                     */
function Rule(name, symbols, postprocess, description, source, preprocess, pkg) {
  var noSuggest = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
...
State.prototype.toString = function () {
  return "{".concat(this.rule.toString(this.dot), ", from ").concat(this.reference || 0); //, context ${this.contextId || 0}`
};
...
Grammar.prototype.expandRule = function (name, pkg, rules) {
  var _this = this;
  var done = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
  if (done[name]) {
    return;
  }
  done[name] = true;
  var parsed1 = name.match(/^(.*?)(?:<(.+)\>)?$/);
  var ruleName = parsed1[1];
  var t = parsed1[2];
  //check if t is a term or an assertion
  var isAssertion = t === null || t === void 0 ? void 0 : t.match(/^(.*?)(?:<(.+)\>)$/);
  if (ruleName === 'literal' && t && !isAssertion) {
    var literal = jsonParse(t);
    if (!literal) {
      throw new Error('Received a literal that is not JSON parsable: ' + t);
    }
    rules.push(new Rule('literal.' + t, tokenize(literal), null, 'a literal rule', 'literal', null, 'generated'));
    return;
  }
  var rulesByName = [].concat(_toConsumableArray(this.byName[name] || []), _toConsumableArray(this.byName[ruleName] || []));
  rulesByName.forEach(function (rule) {
    //if not specialized - just add rule
    if (!t && !(0, _expandRule.isExpandable)(rule)) {
      rules.push(rule);
      return;
    }
    if (!isAssertion && (0, _expandRule.isExpandable)(rule)) {
      //this is a simple term.  - expand the rule using this term
      rules.push((0, _expandRule.expandRule)(rule, t));
      return;
    }
    if (t && rule.name === name) {
  ...
  var terms = this.assertions.getByAssertion(t, pkg);
  terms.forEach(function (term) {
    rules.push(new Rule(name + '#' + term, [ruleName + '<' + term + '>'], function (data) {
      return data[0];
    }, "generated rule", "generated: ".concat(name, "=>").concat(term)));
  });

  //adding maybe
  var maybeTerms = this.assertions.getByAssertionFuzzy(t, pkg);
  maybeTerms.forEach(function (term) {
    rules.push(new Rule(name + '#' + term, [ruleName + '<' + term + '>', '_'].concat(_toConsumableArray(assumeTokens(term.assumptions, _this.assertions))), function (data) {
      return data[0];
    }, "maybe generated rule", "generated maybe: ".concat(name, "=>").concat(term.toString())));
  });
};
...
Grammar.fromCompiled = function (rules, assertions, start) {
  ...
  var rules = rules.map(function (r) {
    return new Rule(r.name, r.symbols, r.postprocess, r.description, r.source, r.preprocess, r.pkg, r.noSuggest);
  });
...
function tokenize(text) {
  var parsed = text.match(/\w+|\W/gm).map(function (token) {
    return token === ' ' ? '_' : {
      literal: token
    };
  });
...
function assumeTokens(assumptions, store) {
  ...
  var text = '(assuming ';
  text += assumptions.map(function (assumption) {
    return [titleOf(assumption[0].toString(), store), assumption[1].toString().replace(/-|\<|\>/g, ' ').trim(), titleOf(assumption[2].toString(), store)].join(' ');
  }).join('; ');
  text += ')';
  return tokenize(text);
}
function titleOf(entity, store) {
  var rels = store.query('title-of', entity);
  if (rels.length === 0) {
    return entity;
  } else {
